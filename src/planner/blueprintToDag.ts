...
export function blueprintToDag(
  ...
  if (conf.validateCycles) {
    ...
    if (cycles.length > 0) {
      throw new Error(
        `检测到循环依赖: ${cycles.map((cycle) => cycle.join(' -> ')).join(', ')}`
      );
    }
  }
  ...
  // 计算性能指标
  const metrics = conf.includeMetrics
...
function buildTopology(nodes: DAGNode[], edges: DAGEdge[]): TopologyResult {
  ...
  if (order.length !== nodes.length) {
    const unvisited = nodes
      ...
      .map((node) => node.id);
    throw new Error(
      `拓扑排序失败，可能存在循环依赖。未访问的节点: ${unvisited.join(', ')}`
    );
  }
...
function analyzeDependencies(
  ...
  return {
    ...
    maxParallelism: Math.max(
      ...parallelGroups.map((group) => group.nodes.length)
    ),
...
function generateOptimizationHints(
  ...
  // 检查可缓存节点
  const cacheableNodes = nodes.filter(
    (node) => node.metadata.cacheable && node.dependencies.length > 0
  );
 ...
 * 验证 DAG 结构
 */
export function validateDAG(dag: ExecutionDAG): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
