...
## 高信息密度指令词操作指南 v1.0（优化版）

**适用对象**：研发 / 产品 / 运营 / 数据 / 增长团队  
**维护机制**：团队一号位或项目经理双周回顾并更新词表与模板。  
**目标**：让任务“开口即定位，落地可执行”，用可验收结果减少沟通与返工。

**核心结构**：所有任务描述遵循「指令词 + 对象 + 截止时间 + 验收口径」，验收口径必须可量化或可机器判定。  
**执行铁律**：
1. **证据优先**：任何结论要绑定数据、日志或链接。
2. **最小可用**：优先跑通可演示/可调用版本，再逐步优化。
3. **时间盒**：排查与探索设置硬时限，超时立即切换方案或升级。
4. **单点闭环**：指定唯一负责人对结果负责到底，其它人只提供输入。

### 快速上手三步
1. 从总表挑选最贴业务的 10 个指令词作为 v1，并沉淀卡片（定义 / 场景 / 模板 / 反例）。
2. 将卡片嵌入 Issue、PR、周会与变更单模板，确保每个任务填写四要素。
3. 每两周复盘采用率、返工率与踩坑案例，调整词表、模板与红线样本。

### 指令词矩阵（分类速查）
**A. 排查与定位（14 词）**  
- 顺藤摸瓜：沿迹象 → 线索 → 根因，交付最小可复现时间线与证据。  
- 最小复现：沉淀最小复现脚本，包含输入 / 输出 / 期望对照。  
- 二分定位：30 分钟内锁定问题归属的切断点。  
- 对比实验：构造仅差单变量的 A/B，固定数据与种子。  
- 旁路验证：用离线或镜像数据复核关键指标。  
- 抓主矛盾：列出 Top3 因素并先解决影响最大的 1 个。  
- 追根溯源：回溯变更时间线，标注每次影响。  
- 画靶定位：先明确靶心样例 / 阈值，再列偏差清单与修正顺序。  
- 复现隔离：在干净环境排除插件 / 缓存等噪声。  
- 热区优先：优先压测热路径接口 / SQL，输出 P95 / P99。  
- 一致性校验：对多源数据跑校验，沉淀差异 TopN。  
- 逆向桩：从异常输出倒查输入，补桩并提交日志。  
- 样本回放：收集异常样本回放对比指标。  
- 影子数据集：沉淀问题样本，纳入回归红线。

**B. 优先级与取舍（10 词）**  
- 止血优先：先控损，P0 1 小时见效，P1 次日 10:00 给方案。  
- 临界路径：列出决定交付日期的关键任务，非临界延后或转派。  
- 二八聚焦：保留最能交付价值的 20% 功能。  
- 砍尾巴：列出长尾低性价比项并说明不做影响。  
- 价值对赌：以指标承诺换取资源优先级。  
- 沉没成本止损：触发退出条件即止损，切换方案 B。  
- 双轨探索：并行 A/B 思路，各自时间盒 1 天后择优。  
- 粗粒度发布：合并小变更在固定窗口发版并列清单。  
- 削峰填谷：高消耗任务错峰执行，监控资源利用。  
- 三色状态：以红 / 黄 / 绿汇报状态与阻塞点。

**C. 交付与执行（12 词）**  
- 纵切贯通：优先交付可跑通的端到端 Demo。  
- 冒烟先行：上线前跑完冒烟清单再进灰度。  
- 先跑通后优雅：演示可用优先，优化排后续迭代。  
- DoD 对齐：在任务中写清完成定义（脚本 / 日志 / 截图）。  
- 时间盒：调参 / 排查设置硬时限，到点切方案或升级。  
- 单点闭环：指定 Owner 对结果负责，他人只供输入。  
- 黄金路径：先把主路径体验打磨到位。  
- 版本锚点：明确基线 tag 及差异摘要。  
- 接口契约：契约冻结，变更需评审并提升版本号。  
- 断点续跑：长流程支持断点续跑与幂等校验。  
- 可被调用：交付成果必须提供 API / CLI 调用示例与返回契约。  
- 现场演示：关键里程碑现场演示黄金路径并录屏归档。

**D. 沟通与协同（8 词）**  
- 拉齐预期：会前发送材料，会中对齐目标 / 边界 / 口径。  
- 反向复述：由接收方复述任务的人 / 事 / 时 / 验收。  
- 证据说话：每个结论后附一条证据链接。  
- 一号位拍板：明确最终拍板人及备选方案与取舍。  
- 双向确认：指派与接收双方共同复核。  
- 前置对齐：事前说明关键假设与不做项。  
- 决策日志：在 Issue 记录决策、时间与依据。  
- 群内可视：关键信息在群内公开汇报。

**E. 风险与质量（10 词）**  
- 预案三件套：上线前交付回滚 / 监控 / 兜底方案。  
- 金丝雀发布：小流量（5% → 20%）守指标 24h 再扩量。  
- 熔断阈值：定义自动降级条件（如 P99 > 800ms 或错误率 > 1%）。  
- 影子流量：镜像请求验证 24h 并提交差异报告。  
- 可观测三件套：补齐日志 / 指标 / 追踪并接入看板。  
- 回归红线：把故障样本纳入必测集。  
- 合规扫描：上线前通过隐私 / 版权 / 密钥扫描。  
- 权限最小化：收敛权限并保留审计记录。  
- 灰度开关：新特性可动态开关，默认策略清晰。  
- 配置托管：集中管理配置并版本化可回滚。

**F. 复盘与成长（6 词）**  
- 时间线复盘：事故后 24h 内串事实时间线。  
- 归因树：提交三级归因树并区分可控性。  
- 可教时刻：沉淀 3 条可复制教训到 Wiki。  
- 知识回填：同步 SOP、脚本与告警规则。  
- 指标闭环：月度复盘命中 / 未命中与纠偏动作。  
- 退出条件：定义停手边界（例：两周无显著提升即停）。

### 可复制模板
1. **任务下发**：标题 = 指令词 + 对象 + 期望；描述 = 目标 / 边界 / 不做项 / 依赖 / 风险 / 验收；时限含时区；责任 = Owner + 拍板人；产出 = 链接 / 脚本 / 截图 / 指标。  
2. **故障排查**：按“止血优先 → 抓主矛盾 → 二分定位”推进，5 分钟一次红黄绿状态同步。  
3. **版本上线**：提交预案三件套、金丝雀计划、熔断阈值、监控看板链接与回滚步骤。  
4. **周度复盘**：记录目标 / 达成 / 偏差 / 纠偏，下周黄金路径与临界路径风险，可教时刻与知识回填。

### 红线与度量
- 红线：不得缺失验收口径、无证据结论、无时间盒探索、多责任人共享、无指标灰度、复盘只甩锅等。  
- 关键度量：指令词采用率 ≥ 80%，Lead Time 周环比、返工率、MTTR、首个纵切 Demo 用时。

### 工具化建议
- 模板校验脚本：检查是否包含指令词 / 截止 / 验收 / Owner / 拍板人，可参考正则：
  ```text
  (?s)(指令词|顺藤摸瓜|金丝雀).*?(截止|到期|\d{4}-\d{2}-\d{2}).*?(验收|脚本|日志|截图)
  ```
- 看板字段：指令词、Owner、状态（红 / 黄 / 绿）、验收链接、退出条件。  
- 归档：演示录屏、日志、对比实验记录、决策日志统一存档。

> 建议打印《Top10 指令词速查表》和《上线三件套清单》，贴在工位与群公告强化记忆。
